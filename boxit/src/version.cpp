...
bool Version::operator<(Version const& version) const {
    return (alpm_pkg_vercmp((*this).toStdString().c_str(),
                            version.toStdString().c_str()) == -1);
}
...
bool Version::operator==(Version const& version) const {
    return (alpm_pkg_vercmp((*this).toStdString().c_str(),
                            version.toStdString().c_str()) == 0);
}



bool Version::operator<=(Version const& version) const {
    int ret = alpm_pkg_vercmp((*this).toStdString().c_str(),
...
/*
 * Soure comes from libalpm - Pacman Development Team <pacman-dev@archlinux.org>
 */
 ...
 */
void Version::parseEVR(char *evr, const char **ep, const char **vp,
        const char **rp)
{
    const char *epoch;
    const char *version;
 ...
 *       -1: b is newer than a
 */
int Version::rpmvercmp(const char *a, const char *b)
{
    char oldch1, oldch2;
    char *str1, *str2;
    char *ptr1, *ptr2;
 ...
 */
int Version::alpm_pkg_vercmp(const char *a, const char *b)
{
    char *full1, *full2;
